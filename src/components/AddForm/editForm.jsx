...
const EditForm = (props) => {
  ...
  const [estimation, setEstimation] = useState(props.editingUser.rating);
  ...
  return (
    <div>
      <input
        type={"text"}
        className="form-control inputEditForm"
        value={name}
        placeholder="Имя"
      ...
      />
      <button className="btn btn-primary btn-sm w-100" onClick={onClickSubmit}>
        Редактировать
      </button>
    </div>
  );
